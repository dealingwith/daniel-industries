...
ssh_port = "22"
document_root = "~/daniel.industries/"
rsync_delete = true
...
source_dir = "~/code/daniel-industries"    # source file directory
...
desc "Create a new page in #{source_dir}"
...
task :isolate, :filename do |t, args|
  if args.filename
    ...
    filename = get_stdin("Enter a file string to filter by: ")
  end
  stash_dir = "#{stash_dir}/"
...
namespace :deploy do
  desc "Dry-run deploy website via rsync"
  task :dry_run => :generate do
    Rake::Task[:copydot].invoke(source_dir, public_dir)
    Rake::Task[:rsync].execute(:dry_run => true)
  end
end
...
task :rsync, [:dry_run] do |t, args|
  abort("Missing DEPLOY_SSH_USER. Add it to .env or export it before deploying.") if ssh_user.to_s.empty?

  exclude = ""
  if File.exist?("./rsync-exclude")
    exclude = "--exclude-from='rsync-exclude'"
  end
  delete_arg = rsync_delete == false ? nil : "--delete-delay"
  diagnostic_args = []
  diagnostic_args << "--dry-run" if args[:dry_run]
  diagnostic_args << "--itemize-changes" if args[:dry_run] || ENV["ITEMIZE"] == "1"
  rsync_options = ["-avze 'ssh -p #{ssh_port}'", exclude, rsync_args, delete_arg, diagnostic_args].flatten.compact.reject(&:empty?).join(" ")
  rsync_command = "rsync #{rsync_options} #{public_dir}/ #{ssh_user}:#{document_root}"
